...
## ✅ 2026-07-09 18:20 — OCR dy 参数校准

**问题**：dy=160 范围过宽导致标签误匹配隔壁字段值块，识别率下降。

**修复**：dy 从 160 回退到 120（80太窄会漏，160太宽会错，120取折中）。X 对齐保持 60。

**最终参数**：dy ≤ 120px，X 对齐 ±60px，只搜下方。

---

## ✅ 2026-07-09 18:30 — OCR X 对齐放宽 + 最终参数确认

**X 对齐** 60→100px，配合 dy=120px。
...
## ✅ 2026-07-10 11:00 — OCR 完全回退到工作版本 + 调试日志

**问题**：07-09 第三方改动后经多轮参数调整仍不如原始版本，识别率持续下降。

**修复**：
1. `_extract_region_values` 和 `_proxy_baidu_ocr` 回退到 07-09 之前的原始代码
2. 仅保留参数调整：dy 80→120，X 60→100
3. 移除所有第三方添加的逻辑（_HORIZONTAL_LABELS / _is_value_block / 双策略 / allow_cjk）
4. 新增 OCR 配对 stderr 日志，方便排查
...
## 🔴 P0 — OCR 多值串拆分 + 左右脑值复制 Bug（2026-07-11）

### 问题描述

精神功能（沟通管理/计划判断 → 创造领导/目标憧憬）、思维功能（逻辑推理/语言功能 → 空间心像/构思拟想）、听觉功能（听觉辨识/语言理解 → 听觉感受/音乐欣赏）出现左侧值复制到右侧的 Bug。

**现象**：
| 功能 | 预期 | 实际 |
|------|------|------|
| 精神沟通 | "20 Wsc" | "20 Wsc 22 Wc"（多值串未拆分）|
| 精神创意 | "22 Wc" | "20 Wsc 22 Wc"（从左侧整串复制 ❌）|
| 思维逻辑 | 应有独立值 | "17 Wc"（从空间心像复制 ❌）|
| 听觉感受 | 应有独立值 | "20 Wc"（从听觉辨识复制 ❌）|
| 体觉 | 正常 ✅ | 正常 ✅ |
| 视觉 | 正常 ✅ | 正常 ✅ |

### 根因分析

`_extract_region_values()` 三个步骤之间存在数据流断裂：

**Bug 1（致命）— Step 3b 拆分多值串后 result 未同步**

```
Step 3:  merged_pairs → result   ← "20 Wsc 22 Wc" 写入 result
Step 3b: merged_pairs 拆分为两个值 ← 只改 merged_pairs，未更新 result！
Step 3c: 读 result 发现 right key 缺失 → 整串复制 ❌
Step 5b: 想拆分已来不及 → right key 已被 3c 污染 → 跳过 ❌
```

**Bug 2 — Step 3b 的 `break` 只处理第一对多值串**

遇到第一对匹配就退出循环，后续多值对被忽略。

**Bug 3 — Step 3c 无条件复制左右脑值**

体觉辨识 ≠ 体觉感受，逻辑推理 ≠ 空间心像 — 左右脑是不同功能，OCR 缺失的值不应从对侧复制。

### 修复内容

#### 修复 1：Step 3b 拆分后同步更新 result + 移除 break

**文件**：`server.py` `_extract_region_values()` 第 236-297 行

- 拆分后直接调用 `result[this_key] = val1` / `result[other_key] = val2` 同步更新
- 移除 `break`，遍历所有多值对
- 新增同 key 标签过滤：`_LABEL_TO_KEY.get(other_label) == _LABEL_TO_KEY.get(label)` 跳过，避免把 val2 赋给同 key 的另一标签

#### 修复 2：移除 Step 3c 左右脑复制逻辑

**文件**：`server.py` `_extract_region_values()` 第 299-301 行

- 整段删除 step 3c 的 sibling key 复制循环
- 左右脑各功能独立，各自的值必须分别识别

#### 修复 3：Step 5b 兜底拆分也移除 break

**文件**：`server.py` `_extract_region_values()` 第 341 行

### 💡 设计原则

- **多值串拆分在 Step 3b 完成**，同时更新 merged_pairs 和 result
- **Step 5b 是兜底**：处理 3b 未能拆分的情况（如找不到 peer label）
- **绝不复制作弊**：缺失值保持空，让用户/OCR 重新识别，而不是用错误数据填充

### 📋 涉及文件

| 文件 | 变更 |
|------|------|
| `server.py` | `_extract_region_values()` Step 3b/3c/5b 三处修复 |

### 验证

- [x] Python 语法验证通过
- [ ] 精神功能：沟通管理=20Wsc, 创造领导=22Wc（拆分正确）
- [ ] 思维功能：不再从对侧复制（缺失侧为空等待重新 OCR）
- [ ] 听觉功能：不再从对侧复制
- [ ] 体觉功能：不受影响（本身两值都能识别）
- [ ] 视觉功能：不受影响
