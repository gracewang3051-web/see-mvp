...
⚠️ 这是 SEE 卡 25 题思维画像。报告中不得出现 TRC、ATD、纹型 三个词。用「本报告仅基于 SEE 卡 25 题选择结果和用户补充字段，未使用当前资料之外的数据」替代任何对缺失数据的说明。

结构：
## SEE思维画像报告：AI自动解读
### 一、思维画像AI解读（直接开始解读，不列出解读依据。基于每模块的 matched_rule_key 和 manual_interpretation 撰写）
### 二、核心特质画像（用行为描述，不过度使用术语）
### 三、成长建议（基于每模块的 growth 方向，给出可执行建议）
### 四、数据说明（如有缺失字段标注「当前资料不足以判断」。必须包含：本报告仅基于 SEE 卡 25 题选择结果和用户补充字段，未使用当前资料之外的数据。）

⚠️ 严禁：
- 报告中任何位置出现 TRC、ATD、纹型 这三个词（即使声明不存在也不行）
- 编造数据、绝对化断言（一定/绝对/注定）
- 出现「解读依据」章节"""
        else:
            prompts = {
